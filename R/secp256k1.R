# File: R/secp256k1.R
# secp256k1 over gmp bigz: public keys, deterministic ECDSA (RFC 6979) with
# Ethereum low-s + recovery id, and public-key recovery (ecrecover). Pure R on
# base + gmp + openssl.
...
#' Coerce a Scalar to a `gmp::bigz`
#'
#' Accepts a `gmp::bigz`, an integer/double, a `0x`-prefixed hex string, or
#' `raw(n)` big-endian bytes. Used by [ecrecover()] and the signature builder so
#' callers can pass any of the natural signature-scalar representations.
#'
#' @param x A `gmp::bigz`, numeric, hex string, or raw vector.
#' @return A scalar `gmp::bigz`.
#' @importFrom gmp as.bigz
#' @keywords internal
#' @noRd
as_scalar_bigz <- function(x) {
  if (inherits(x, "bigz")) {
    # Already a bigz: return it untouched. Re-running gmp::as.bigz() on an
    # existing bigz can corrupt the value (it reparses its decimal print form),
    # which silently broke the signature hex encoding.
    return(x)
  }
  if (is.raw(x)) {
    return(raw2bigz(x))
  }
  return(gmp::as.bigz(x))
}
...
#'
#' @importFrom gmp mod.bigz divq.bigz inv.bigz powm
#' @importFrom rlang abort
#' @export
